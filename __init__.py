...
def create_app(test_config=None):
    ...
    def sign_up():
        if request.method == 'POST':
            ...
            if not username:
                ...
                error = 'Username ia already taken.'
    ...
    def note_create():
        if request.method == 'POST':
            ...
            if not error:
                ...
                db.session.add(note)
                db.session.commit()
                flash(f"Successfuly create note: '{title}'", 'success')
    ...
    def note_update(note_id):
        ...
        if request.method in ['POST', 'PATCH', 'PUT']:
            ...
            if not error:
                ...
                db.session.add(note)
                db.session.commit()
                flash(f"Successfully update note: '{title}'", 'success')
                return redirect(url_for('note_index'))
            flash(error, 'error')
    ...
    def note_delete(note_id):
        ...
        db.session.commit()
        flash(f"Successfuly delete note: '{note.title}'", 'success')
        return redirect(url_for('note_index'))
